        ...
        @foreach($data as $record)
            <div class="col-md-4 mb-3">
                <div class="card card-body h-100">
                    <div class="container h-100">
                        @if(array_key_exists('multimedia', $record['_source']))
                            <a href="{{ route('record', $record['_source']['identifier'][1]['priref']) }}">
                                <img class="img-fluid"
                                     src="{{ env('CIIM_IMAGE_URL') }}{{ $record['_source']['multimedia'][0]['processed']['preview']['location'] }}"
                                     loading="lazy"
                                     alt="Preview image for {{ ucfirst($record['_source']['summary_title']) }}"/></a>
                        @else
                            <a href="{{ route('record', $record['_source']['identifier'][1]['priref']) }}">
                    ...
                    <a href="{{ route('record', $record['_source']['identifier'][1]['priref']) }}" class="btn btn-dark">
                        Read about this object
                    </a>
                </div>
            </div>
        @endforeach
