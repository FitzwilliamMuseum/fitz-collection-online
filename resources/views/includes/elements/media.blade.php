                ...
                <div class="shadow-sm p-3 mx-auto mb-3">
                    ...
                    <div class="text-center mt-2">
                        <a class="btn btn-sm btn-sm btn-dark m-1"
                           href="{{ route('image.single', $record['_source']['multimedia'][0]['admin']['id']) }}">@svg('fas-search',['width'
                            => 15])View image details</a>
                            ...
                            @if(Arr::get($con, 'zoom', true))
                                ...
                                <a class="btn btn-sm btn-sm btn-dark m-1 "
                                   href="{{ route('image.iiif', $record['_source']['multimedia'][0]['admin']['id']) }}"><img
                                        src="{{ asset( '/images/logos/iiif.svg') }}" alt="IIIF icon - view image" width="20px"/> Deep
                                    zoom
                                    UV</a>
                                <a class="btn btn-sm btn-sm btn-dark m-1 "
                                   href="{{ route('image.mirador', $record['_source']['multimedia'][0]['admin']['id']) }}"><img
                                        src="{{ asset( '/images/icons/Mirador.svg')}}" alt="Mirador icon - view image" width="20px"/>
                                    Mirador</a>
                                <a class="btn btn-sm btn-sm btn-dark m-1 "
                                   href="https://api.fitz.ms/data-distributor/iiif/{{ $record['_source']['admin']['id']}}/manifest"><img
                                        src="{{ asset('/images/logos/iiif.svg') }}" alt="IIIF Manifest" width="20px"/> IIIF
                                    Manifest</a>
                                <a class="btn btn-sm btn-sm btn-dark m-1 "
                                   href="{{ route('slow.iiif', ['image' => $slow[0]]) }}">@svg('fas-eye', ['width' => 15])
                                    Slow looking</a>
                            @endif
                        @endif
                        @php
                        ...
                        @if(!empty($three))
                            <a class="btn btn-sm btn-sm btn-dark m-1 "
                               href="{{ route('sketchfab', $record['_source']['identifier'][1]['priref']) }}"><img
