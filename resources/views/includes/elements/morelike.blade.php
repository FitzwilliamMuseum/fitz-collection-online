                  ...
                  @if(array_key_exists('multimedia', $record['_source']))
                    @if(array_key_exists('preview',$record['_source']['multimedia'][0]['processed']))
                      <a href="{{ route('record', $record['_source']['identifier'][1]['priref']) }}">
                        <img class="card-img-top" src="{{ env('APP_URL')}}/imagestore/{{ $record['_source']['multimedia'][0]['processed']['preview']['location'] }}" loading="lazy" alt="An image of {{ ucfirst($record['_source']['summary_title']) }}"/>
                      </a>
                    @endif
                  @else
